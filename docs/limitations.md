...
在已验证的库版本中，H100 80GB上41/62/78/111原子体系均曾在解析Hessian路径触发CUDA内存分配失败。独立进程重测确认主要涉及工作区和张量收缩临时副本。即使监控曲线未到100%，后续单次申请大于剩余显存仍会失败。可显式启用[Hessian显存策略](hessian-memory.md)，但其兼容范围受源码哈希限制，也不保证任意体系都能在给定显存下完成。

显存策略是默认关闭的实验功能。分块交叉对照与独立梯度差分是不同的数值检验；62原子体系的独立差分仍未通过1e-5 Eh/Bohr²阈值，收紧SCF、单独更换CPHF网格或关闭SCF密度网格筛除均未消除偏差。详见[本分支验证记录](hessian-memory-validation.md)，不能将不再OOM等同于大体系TSOPT/IRC已获得完整数值验收。
